...
## 2026-08-10 - Prevent SSRF via Local/Internal Domains in DAV Adapters
**Vulnerability:** The `LocalDavAdapters` URL validation logic correctly blocked non-global IP addresses, but failed to block internal domain extensions such as `.internal` or `.local` (or exact matches for `internal` and `localhost`) when validating hostnames before DNS resolution.
**Learning:** Checking for global IP addresses after DNS resolution is not enough. The hostname itself must also be validated against internal domains like `.internal`, `.local`, and `localhost` to prevent SSRF bypasses before DNS resolution.
**Prevention:** Always explicitly check and block domains matching `.internal`, `.local`, `internal`, and `localhost` when validating URLs for global reachability to prevent SSRF bypasses.
