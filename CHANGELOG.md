## [Unreleased]
- Reply-SLA contention failures now carry stable machine-readable codes for an
  unresolved duplicate race, exhausted batch retry budget, and a source email
  that left the caller's scope. The HTTP 409 boundary preserves the specific
  code while keeping its customer-safe message independent of internal details.
