## [2026-09-09] - 날짜 계산기 도구 추가

### Added
- 사용자가 입력한 기준 날짜로부터 지정한 일수만큼 날짜를 더하거나 빼는 `date_calculator` 도구를 추가했습니다.
- 백엔드에 날짜 계산 핸들러(`date_calculator_handler`) 및 API 라우트를 추가했습니다.
- 테스트 커버리지를 100%로 유지하기 위한 도구의 단위 테스트를 구현했습니다.
