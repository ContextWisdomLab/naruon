## [Unreleased]
- OpenCode 모델·MCP 정책의 저장소별 복사본을 제거했습니다. Naruon의 required
  review는 중앙 workflow가 격리 경로에 주입하는 단일 `opencode.jsonc`만
  사용하며, release-governance 테스트가 두 OpenCode JSON 파일의 재도입을
  막습니다.
