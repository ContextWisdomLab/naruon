## [Unreleased]
- **Performance (Bolt)**: `NetworkGraph` 컴포넌트 내의 관계 및 노드 선택 옵션 생성 시 `.map().filter().slice()` 등 체인된 배열 메서드 사용을 단일 루프(bounded `for...of`)로 대체하여 불필요한 O(N) 메모리 할당 및 병목 현상을 방지했습니다.
