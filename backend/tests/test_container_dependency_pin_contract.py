...
def test_container_provenance_dependency_pins_match_reviewed_manifests() -> None:
    ...
    assert backend_pins["cryptography"] == "50.0.1"
    assert backend_pins["protobuf"] == "7.35.1"
    assert "cryptography==50.0.1" in backend_records
    assert "protobuf==7.35.1" in backend_records
    assert all(
        re.fullmatch(r"[0-9a-f]{64}", digest)
        for pin in ("cryptography==50.0.1", "protobuf==7.35.1")
        for digest in backend_records[pin]
    )

    assert strix_pins["cryptography"] == "50.0.1"
    assert strix_pins["protobuf"] == "6.33.6"
    assert "cryptography==50.0.1" in strix_records
    assert "protobuf==6.33.6" in strix_records
    assert all(
        re.fullmatch(r"[0-9a-f]{64}", digest)
        for pin in ("cryptography==50.0.1", "protobuf==6.33.6")
