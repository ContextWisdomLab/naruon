...
| #1590 | `57bfd564a3805a512b347538817362dda7c77a62` | current hosted image validation remains in progress; predecessor focused tests were run on `39158a5e` | the URL extractor boundary delta is locally verified, but repeated empty acknowledgement commits add no effective change and invalidate predecessor-head hosted evidence. Stop empty requeues; preserve the valid delta and require a fresh meaningful head before review or merge |
| `.github#2035` | `6324f4dce523039c992956d478750d74f55e52e3` | Noema run `34183702988`, job `101940582259` | CO sidecar returned HTTP 502 after 88.5 seconds with `phase=response_error` and `served_model=unknown`; this is upstream transport evidence, not a consumer defect. Provider/failover repair belongs to CO; no paid consumer fallback or failed-verdict downgrade |
