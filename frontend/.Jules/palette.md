...
## 2024-05-20 - Accessible tooltips and onClick event parameter dropping
**Learning:** When migrating buttons from the native `disabled` attribute to `aria-disabled` for accessibility, replacing `disabled` prevents the browser from automatically blocking click events. Consequently, you must intercept the `onClick` event and manually prevent the default action using `e.preventDefault()`. However, when doing so, be careful not to accidentally drop the `e` parameter if the original handler expects it (e.g. `handleRunnerTokenRotate(e)` instead of `void handleRunnerTokenRotate()`). Furthermore, remember to update associated unit tests that might still assert `element.disabled === true` to check for the `aria-disabled` attribute instead.
**Action:** Always maintain the exact signature of the original `onClick` callback when intercepting clicks for `aria-disabled` buttons, and proactively update any `element.disabled` checks in tests to `element.getAttribute('aria-disabled') === 'true'`.
