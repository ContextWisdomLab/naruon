...
describe("TasksLayout", () => {
  ...
  it("keeps the first task when reply SLA results repeat an existing id", async () => {
    const existingTask = {
      id: "task-duplicate-1",
      title: "기존 작업 제목",
      status: "open",
    ...
    const fetchMock = vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.endsWith("/api/tasks/reply-sla-escalations")) {
        ...
        return Promise.resolve(jsonResponse({
          ...
          tasks: [{ ...existingTask, title: "새로 반환된 중복 제목", status: "in_progress" }],
        }));
      }
      throw new Error(`Unexpected fetch: ${url}`);
    });
    ...
    const ticketList = container.querySelector('[aria-label="원본 연결 티켓 목록"]');
    expect(ticketList?.textContent).toContain("기존 작업 제목");
    expect(ticketList?.textContent).not.toContain("새로 반환된 중복 제목");
    expect(ticketList?.querySelectorAll("article")).toHaveLength(1);
  });
});
